...
public class RegisterDeliveryController {
	...
	private void registerDelivery() {
		...
		String newStatus = model.registerDelivery(shipmentId, result, simulatedDate, notes);
		...
		else if ("AT_OFFICE_FOR_PICKUP".equals(newStatus))
			JOptionPane.showMessageDialog(view.getFrame(),
					"Delivery failed (" + (shipment.getFailedAttempts() + 1) + " attempts).\nThe package remains at "
							+ shipment.getDestinationLocation() + " and the customer has been advised.",
					"Delivery Failed", JOptionPane.WARNING_MESSAGE);
		else
			JOptionPane.showMessageDialog(view.getFrame(),
					"Recipient absent (attempt " + (shipment.getFailedAttempts() + 1)
